...
class CACv3(CAC):
    ...
    def optimize_W_params(self, primal_loss: torch.Tensor, dual_loss: torch.Tensor):
        ...
        warmup_complete = self.progress >= 0.1
        ...
        grad_dict.update(
            self.compute_gradient_norm(
                [self.W_func, self.lbd, self.w_ub, self.w_lb],
                ["W_func", "lbd", "w_ub", "w_lb"],
                dir=f"{self.name}",
                device=self.device,
            )
        )
    ...
    def learn_W(self):
        ...
        loss_dict = {
            ...
            f"{self.name}/lr/w_ub_lr": self.lr_scheduler1.get_last_lr()[2],
            f"{self.name}/lr/w_lb_lr": self.lr_scheduler1.get_last_lr()[3],
